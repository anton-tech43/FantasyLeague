...
| **P1** | A5 | En tidszon i all kopia. Mätt: 25 items med klockslag, 5 anger zon; alla UK-tid → svensk läsare 1 h fel. **Push-sidan löst 6 sep (skriven, ej deployad): mig 082** `device_tokens.timezone` + `register_device_token(p_timezone)`; iOS skickar `TimeZone.current.identifier`; `matchday-reminder` och `morning-push` renderar avspark per läsarens zon (en payload per zon). **Default/fallback = Europe/London** (Antons beslut 6 sep: appen marknadsförs och betalas i UK; de ~20 befintliga svenska enheterna får London-tid tills iOS-builden som skickar zon är ute). Fler marknader senare → per-enhet-zon i stället för fast zon. **Routine-sidan kvarstår:** regel i PROMPT/SUNDAY_BRIEF att skriva tid med zon eller relativt ("tonight"). | app/routines |
