...
- **Backfill in progress:** sequential per-team regeneration of all 71 team_pages rows running in background — synchronous all-71 invocation timed out at the Edge Function gateway, so falling back to a team-by-team loop locally. Smoke-tested on arsenal (20-row PL table, 4 fixtures with 2-5 dot ratings) and sweden (4-row Group F table, 7 fixtures, "Pre-World Cup tune-up" tag). Expected ~65+ teams have standings + upcoming_fixtures after the loop completes.
