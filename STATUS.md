...
**2026-09-22 — Lingo redesigned as Overheard (ships in the next iOS build):** the 12-level glossary + flip-card drill is gone. Lingo now deals "This weekend's words", seven terms picked on the phone from the cached team page (derby, cup, table position, the last result), played as Overheard: the line she hears, three meanings, then the friend's explanation and the line she says back. All 158 terms carry hand-written `overheard`/`gist`/`decoys`/`when` (`tools/myturn/lingo_overheard/`, validator-gated: no club names, no living people, no answer-word leaking from the bubble, no length tell). No streaks, no dailies. Design + rules: `tools/myturn/CONTENT_PRINCIPLES.md` §7, `tools/myturn/LINGO_OVERHEARD_BRIEF.md`.

**2026-09-16 — Meta SDK for app-install ads (ships as 2.2 / build 11):** `FacebookCore` via SPM (app events + SKAdNetwork, no Login) added with `ios/scripts/add_facebook_sdk.rb`, Meta App ID + client token + the two SKAdNetwork ids in `Info.plist`, and a once-per-install ATT prompt 1.5 s after the tabs appear — without both halves Meta caps our install ads at iOS ≤ 14.4.

**Last updated:** 2026-06-11 (**Lesson 99** — Live Activity for live WC matches (Lock Screen + Dynamic Island, push-to-start). The half-time brief lands in `live_match_briefs` (in-app only); a Live Activity is the correct on-device surface for evolving match state (vs a one-shot push). Built across iOS 2.1 + backend, all building/deployed, verified live on the opener (match-watcher clean; `live-match-current` returned Mexico 2-0 SA 2H; drive state `la_started=t`). iOS: new Widget Extension target `GoalDiggerLiveActivity` added via `ios/scripts/add_live_activity_target.rb` (xcodeproj gem — never hand-edit pbxproj), `MatchActivityAttributes` + Live Activity UI (flag emoji, no remote crests, no App Group needed), `LiveActivityManager` (push-to-start + per-activity update tokens + foreground fallback), `NSSupportsLiveActivities`. Backend: mig 062 (`live_activity_tokens` + `match_status_state.la_started/la_sig/la_ended`), `sendLiveActivityPush` in apns-client (reuses the ES256 JWT; liveactivity push-type), match-watcher START/UPDATE/END hooks (WC league 1, period-based label to keep APNs volume sane, idempotent), `live-match-current` endpoint. WC-only v1; scope = score + period (no goal scorers). **Needs a physical device to fully verify** (sim can't do remote ActivityKit pushes) + ships in the 2.1 build. **Lesson 98** — fixed "some users re-enter all onboarding on relaunch": onboarding state lives in `UserDefaults.standard` (synchronous read, plain `hasCompletedOnboarding` gate), but UserDefaults flushes to disk lazily via cfprefsd — `completeOnboarding()` never forced a flush, so a user who finishes onboarding then force-quits (or crashes) before the async flush loses *every* field and re-onboards. Fits "re-enter everything / some users every time / not on my phone" (force-quitters + crashy devices never flush; normal backgrounding does). Fix (iOS 2.0.1, builds clean): `AppState.persistNow()` → `UserDefaults.synchronize()` called at onboarding completion (flushes the whole suite atomically) + on scenePhase resign-active; also fixed `selectedTeam` nil-removal asymmetry; DEBUG presence-log in RootView for TestFlight confirmation. **No server lever** (names are local-only) — ships in 2.0.1, affected users keep losing state until approved. Ruled out: decode-reset, Caches-purge, async race, background-locked read, accidental reset. **Lesson 97** — whole-app WC data-correctness audit on kickoff day + a freshness lens ([WC_DATA_AUDIT_2026-06-11.md](./WC_DATA_AUDIT_2026-06-11.md)). Free in-session (12 web-verification agents, one per group; no paid loop). **Managers 48/48 correct** — six I'd have "fixed" from memory (Morocco/Ouahbi, Ghana/Queiroz, Tunisia/Lamouchi, Saudi/Donis, Uruguay/Bielsa, Curaçao/Advocaat) were all real Mar–May 2026 changes already right; the 12 `COACH_OVERRIDES` vindicated; only Qatar name completed → Julen Lopetegui. **Fixtures:** all 12 groups + matchups correct; the "dates off by a day" flags were UTC-vs-host-local rendering (stored instants correct, iOS renders device-local), *not* changed. **Real fixture bug (🔴, all 48):** `updateWcDynamicFields` parsed `fixtures_next` with no competition filter, so post-tournament Nations League / qualifier games leaked into `upcoming_fixtures` (England 3 WC + 6 NL), some mislabeled "Group stage game" 4-dots when an opponent name matched a group rival → fixed with tested `filterFixturesByLeague` + `parseUpcomingFixtures(..., WC_LEAGUE_ID=1)`; post-deploy every team shows exactly its 3 group games. **Players:** 4 featured players weren't in the final squad (En-Nesyri/Morocco omitted, Mitoma/Japan injured, Palmer/England cut, Al Naimat/Jordan ACL) → replaced with verified members (El Kaabi, Kamada, Saka, Olwan); ~41 one-liner fixes across 33 countries (stale clubs Son→LAFC, McTominay→Napoli, Kudus→Spurs, Partey→Villarreal, Semenyo→Man City, Xhaka→Sunderland, Kessié→Al-Ahli, Taremi→Olympiacos, Luis Díaz→Bayern, …; ages Ronaldo 41/Džeko 40; Larin not top scorer; Souček not captain; 2 Egypt em-dashes cleaned). Durable (SQL on `ones_to_know.players`; dynamic_only preserves it, no full-regen cron); refresh propagated fixes into opponents' Coming-up blocks. **Freshness lens:** migration 061 `v_wc_page_freshness` + `get_wc_freshness()` RPC + `scripts/wc-freshness.sh` (tier-aware: dynamic cards ~2h SLA vs static LLM cards days-old; stale >14h to skip the overnight gap); 48/48 fresh. Lessons: the "fix" instinct is the hazard on discretely-changing facts; a timezone gap is not a data error; an entity-scoped feed can still carry out-of-competition rows. **Lesson 96** — Coming-up card polish + player-club staleness + the archive cron that ate the previews. Breathing room between opponent players shipped server-side; bold names/photos = structured opponent block in the Coming-up card (iOS 2.0.1, built). Calendar: fixed re-add AND first-time-add failures (stale EKEventStore + nil default-calendar source) via store.reset, robust writableSource fallback, error propagation, full-schedule sync, "Re-add" button (2.0.1, built). Player one-liners broadly stale (summer-2025 transfers): 144-player web sweep → corrected heavy-traffic markets (Sweden Gyökeres→Arsenal/Isak→Liverpool/Lindelöf→Villa, Germany Wirtz→Liverpool, Croatia Modrić→Milan, Canada David→Juventus, Belgium Openda→Juventus); durable (no full-regen cron). **Found the real preview-killer:** cron `goaldigger-archive-old-content` (daily, archives published >7 days) was re-archiving England's previews nightly and would have deleted USA's mid-tournament — fixed to exclude `preview_fixture_id IS NOT NULL` (+ gave overviews synthetic ids). Uncommitted iOS (calendar + structured opponent) ready for 2.0.1. **Lesson 95** — API-Football's national-team coach data is broadly stale: the app showed Sweden's sacked coach (Tomasson, gone Oct 2025) and 3 `<UNKNOWN>` managers. A web-verified sweep of all 48 WC countries found **12 wrong/missing (25%)** — incl. Spain showing *France's* Deschamps, fabricated names for South Africa/Senegal, and Ghana/Tunisia/Saudi/Czech/NZ all stale. Fixed via a verified `COACH_OVERRIDES` map (name + real photo + summary) applied deterministically every refresh (survives full regen): Sweden→Potter, France→Deschamps, Scotland→Clarke, Uruguay→Bielsa, Spain→de la Fuente, Ghana→Queiroz, Tunisia→Lamouchi, South Africa→Broos, Senegal→Thiaw, Saudi→Donis, Czech→Koubek, NZ→Bazeley. Country manager photos now **48/48, 0 UNKNOWN**; the Tunisia fix also corrected Sweden's opponent blurb. Not yet audited: player ones_to_know + PL club coaches. **Lesson 94** — opponent context on the team page, the user's idea to *reuse the opponent's own curated card rather than rewrite it*: Sweden vs Tunisia now shows Tunisia's manager + danger men, pulled from Tunisia's own page, so it appears in both feeds and updates per fixture. **Live server-side, no build, all 48 countries:** an opponent blurb appended to the "Coming up" card's `preview` (`renderOpponentBlurb`), plus a forward-compatible `ones_to_know.opponent` block (opponent's players + photos, ignored by current clients). **2.0.1 (coded + compiles):** those opponent players render *inside* the existing "Ones to know" card under an "Up next: <Team> / Their ones to watch" header (a build is needed only because the live app hard-caps that card at 3 players and has no opponent label). Also hardened the WC next_fixture refresh: **newest-good-wins** on `fixtures_next` (a transient empty fetch had blanked Sweden's rebuild) + **authoritative played-detection** via `fixtures_last` (new tested `_shared/fixture-rollover.ts` — `collectFinishedFixtureIds`/`dropFinished`) so a stale snapshot can't surface a played game as "next"; the card recomputes the next opponent each refresh (rollover on the ~2h data-fetcher cadence; not live-verifiable until WC group games are played, covered by 5 unit tests). **Lesson 93** — US-market parity: comparing England vs USA proved it was a **content gap, not a code gap** (engine is fully country-agnostic, no `england` hardcoding). Seeded **4 hand-curated USA previews** (Group D overview + Paraguay/Australia/Türkiye), gold-standard depth, accurate opponent players from each opponent's own card, feed-only, calendar-tappable via `preview_fixture_id` — **server-side only, no build** (the iOS Calendar-tap feature shipped in V2.0). Republished England's 4 previews (they'd been **archived** as collateral of a blanket `archive WHERE pipeline_source='routine'` cleanup; `'manual'` is impossible — CHECK allows only `edge_function`/`routine`). Discovered the "47/48 empty" note is **stale**: a routine seeded ~14 countries with lighter previews around Jun 3-4 but **skipped 34**, including Brazil/Argentina/France/Germany/Spain and **both co-hosts Canada + Mexico** — the same no-build lever can extend to them (out of the USA-only scope chosen). WC group-stage shipped, kickoff Jun 11. **Lesson 90** — the "removed from App Store" scare was NOT a rejection: V2.0 (build 1.0(3)) passed review but App Availability had emptied + EU DSA trader status was unset. Set availability for the **148 non-EU regions (incl. UK)** → live for kickoff; the EU-27 auto-join once trader status (now under Apple review) verifies. **Lesson 91** — WC team pages were going **8 days stale** (the cheap `dynamic_only` refresh bailed out for countries → paid weekly Claude regen); fixed with a **deterministic WC refresh** (`updateWcDynamicFields`, zero Claude) + a **stakes engine** (per-fixture importance dots + "what they need" labels) + factual **WC_RIVAL_RESULT** & deterministic **post_match** pushes (mig 060); **PL pushes flipped to feed-only** for the tournament window (`post_news.sh`, routines `4141fbd`). Verified: 48/48 countries fresh, 0 stale fixtures, 1-2s refreshes. **Lesson 92** — **exact group-stage qualification math**: `group-scenarios.ts` (brute-force ≤3^6 → "Won the group" / "At worst 2nd", tiebreaker-agnostic, sound under the verified 2026 H2H-first rules) + `best-third.ts` (cross-group 8-of-12 → in/out/soft, points-only strict separation + completeness guard) + freshness/completeness gate; **WC_BEST_THIRD_QUALIFIED** pushes, **elimination is in-app only (never pushed)**; **45 Deno tests**; deployed `team-page-generator` + `match-watcher`, **dormant until real games create locked states**. WC **player dataset complete** (48 teams / 1248 players, original copyright-safe notes). Git: app-repo commits are **local (14 ahead of origin, unpushed)**; Edge Functions deployed live; routines pushed. OPEN: **iOS 2.0.1 fast-follow** (native WC group hub + client freshness — `.task(id:)` reload / pull-to-refresh / past-fixture guard) for the next submission. Earlier — Content quality: Lesson 88 — `content-audit` deterministic standings-claim linter shipped (Edge fn + `_shared/audit-claims.ts` + mig 059 nightly cron; $0, zero false positives on the live corpus, 18 unit tests) after the West Ham "stayed up while 18th" miscount. Lesson 89 — full 577-push 30-day audit (`CONTENT_PUSH_AUDIT_2026-06.md`): system is voice-competent + truthful where checkable, but over-pushes; the TEAM IMPACT + TRANSFER gates existed yet were ignored at scale, so the fix was widening the deterministic `post_news.sh` guards (speculation phrasings + the "World Cup"→"World Championship" rename that had silently disabled the int'l-duty downgrade + banned-register reject) plus PROMPT.md opener-rotation/saga-dedup + SUNDAY_BRIEF internal-consistency. Routines pushed (41c5fd2). OPEN for user: Liverpool 5th=CL contradiction (needs season's CL allocation); optional notification-sender null-title skip + per-team push throttle. Earlier — 2026-06-02 App Store submission DONE — V2.0 work shipped as build 1.0(3), in Apple review; free pricing, EU DSA trader status + W-8BEN done. Post-submission WC hardening: Lesson 83 (feed-only push side-door closed — push_eligible now honoured by notification-sender's direct trigger, not just the sweep; 5 leaked feed-only pushes found + stopped), Lesson 84 (Disk IO budget — composite index on raw_fetch_logs + 61k-row trim + retention cron, no upgrade needed), Lesson 85 (WC consequence math B1-B3 — disabled false-ELIMINATED, knockout-stage gate, GD-aware re-rank; deployed + math-tested), Lesson 86 (WC pre-launch review — iOS active-entity fixes for feed-reload/insider/quiz/live-brief, England Group-F content_items corrected). Lesson 87: server-side insights layer (migration 058 — aggregate views + get_insights() RPC + scripts/insights.sh dashboard), zero new tracking, honours "we don't track you"; surfaces audience/growth/churn/push-delivery/content + the 13 zero-content WC feeds. OPEN: content gap (47/48 countries no preview feed — gd-wc-preview routine not built; insights confirms 13 fully empty), iOS 2.0.1 fast-follow (active-entity fixes + Phase-B insights push-opens/sessions ship in next build, not the in-review one). Earlier — full pre-launch security sweep (Lessons 79 + 80). Closed: anon-callable service-key RPC (mig 055), two no-RLS tables (mig 054), unauthenticated cost-drain on 11 Edge Functions (require-service-auth gate), device_tokens table-wipe grants (mig 056), unfiltered routine content (post_news.sh injection/PII guard). Every fix verified from the live attack surface; live crons confirmed 200 post-deploy. Wave-2 fast-follows (device_tokens full lockdown, delete-my-data rate-limit) + Wave-3 platform items (rate-limiting, dashboard 2FA, OpenAPI restriction, key-rotation runbook, 47 remaining WC crests) documented + deferred. DB + function layer launch-safe. Post-sweep verification (May 28): iOS secret audit + PostgREST filter-injection both PASS, no exploitable issue, no code change — see Lesson 81. Lesson 82 same day: Arsenal user feedback on 4 pushes → two new force-downgrade patterns in `post_news.sh` (speculation + opaque-teaser) + PROMPT.md TRANSFER PUSH-WORTHINESS gate + HEADLINE CLARITY rule. Tested 13/13. Depth-player threshold flagged as known follow-up.)
...
GoalDigger **V2.0 (WC 2026 support) is submitted to the App Store** (build 1.0(3), in Apple review, free pricing, auto-release on approval). WC kicks off **June 11**. Backend is hardened (security sweep + push-gate + Disk-IO + WC consequence-math fixes, all live). **Two open launch-quality items:** (1) preview-content coverage is **patchy** — 14/48 countries have published per-opponent previews (USA + England hand-curated at gold-standard depth; ~12 others routine-light), **34 still missing** incl. Brazil/Argentina/France/Germany/Spain and co-hosts Canada + Mexico (same server-side/no-build lever can fill them); (2) iOS active-entity bugs (feed not reloading on club↔country switch; insider/quiz/live-brief fetching the club not the country) are fixed in-branch but ship in the **next build (2.0.1)**, not the one in review. England (launch market) is content-complete and correct.
...
| `device_tokens` anon SELECT/UPDATE (`USING true`) — needed for the merge-duplicates registration upsert (migration 030) | 🟠 MEDIUM (griefing, not breach) | Migration 056 revoked anon DELETE/TRUNCATE/TRIGGER/REFERENCES (table-wipe grant gone). SELECT/UPDATE kept (upsert needs them); follow-graph SELECT narrowing → Wave 2 (Edge Function) | partial ✓ |
| 🔴 Unauthenticated Edge Function invocation → Anthropic/API-Football/APNs cost-drain. `team-page-generator` ran with ZERO auth header (gateway `verify_jwt` accepts the anon key, which ships in the app). 11 server-only functions had no internal caller check. | 🔴 CRITICAL | `_shared/require-service-auth.ts` gate on all 11 (commit 9ce0364). Verified: no-auth/anon → 401, service key → runs, live crons → 200 ×N. New secret `CRON_AUTH_KEY` = Vault cron key. | ✅ |
| No safety gate on routine content (content-reviewer off) + RSS prompt-injection → could push injected/PII content unfiltered | 🟠 ORANGE | `post_news.sh` content-safety guard — hard-rejects injection signatures + PII (routines `520b4d3`). 8/8 fixture tests. | ✅ |
| `pg_net` PUBLIC EXECUTE (SSRF defence-in-depth) | 🟡 LOW | Skipped — `net` not REST-exposed (no live exploit) + blanket revoke would break postgres's cron `net.http_post`. Revisit with role-scoped grants post-launch. | n/a |
| OpenAPI introspection, scraping/rate-limit, dashboard 2FA, key-rotation runbook | 🟡 LOW | Wave 3 — platform/runbook, user action. | deferred |
| iOS secret audit + PostgREST filter-injection (post-sweep verification, May 28) | ✅ PASS | No code change. Only `sb_publishable_*` ships (no `.p8`/service/API-Football key; `api-sports.io` refs are the keyless image CDN; real xcconfig gitignored). 3 `.or()` interpolation sites: the one user-input-reachable (`live-brief-current`) is regex-guarded `/^[a-z_]{2,32}$/`; the two server-side (`notification-sender`, `morning-push`) are DB-sourced + behind require-service-auth + anon can't write the source tables. | read-only grep + RLS trace ✓ |
