...
| `{{content_type}}` | content_items.type | "news" or "matchday" (used by all 3 review bots) |
| `{{talking_points_formatted}}` | content_items.talking_points | Talking points as numbered list (regular points only for matchday) |
...
| 2026-03-28 | All (Structural Validation) | P6 — Offline structural validation of all prompts against golden examples and anti-patterns. All 5 golden examples pass all 3 review bots structurally (tone, accuracy shape, brevity limits). All 6 anti-patterns correctly fail at least 1 bot. Contract 6 compliance verified (pass/confidence/notes required in all 3 tools). Contract 3 matchday JSONB fields present and required. Contract 4 newsworthiness_score output verified. Added missing variables to reference: `{{content_type}}`, `{{talking_points_formatted}}`. Live API testing blocked (no ANTHROPIC_API_KEY access). | Validates prompt quality without live API calls; confirms review bots would gate golden examples correctly | Structural pass; live testing pending API key |
