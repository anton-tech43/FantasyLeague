...
- `next_fixture.preview` gets an appended opponent sentence — `renderOpponentBlurb()` in stakes-templates.ts: *"Tunisia are managed by S. Trabelsi, with Hannibal Mejbri and Ellyes Skhiri the ones to watch."* Always names the opponent (never "your"), so the existing "Coming up" card reads it with no app change. Singular/plural + missing-manager branches handled; no em-dashes; "World Championship" not "World Cup".
