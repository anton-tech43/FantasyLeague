...
### 99. Live Activity for live WC matches (Lock Screen + Dynamic Island, push-to-start)

**Trigger:** user wanted the live match update "on the phone," not just in-app. The half-time brief lands in `live_match_briefs` and `notification-sender` only reads `content_items`, so it never reaches APNs — the in-app LiveMatchCard was the only surface (the root of the recurring "live game push" frustration: the data fired, there was no on-device surface). The right answer is an **ActivityKit Live Activity** — a live score on the Lock Screen + Dynamic Island, updated through the match, not a one-shot push. User chose **full auto (push-to-start)**.

**Scope:** WC matches only (`league_id=1`) for v1 — which lets the UI use **country flag emoji** (`Country.flagEmoji`, mirrored server-side in `_shared/wc-countries.ts`) instead of the remote crest CDN, which the widget process can't reliably load. No App Group needed: all display data rides in the activity payload (attributes + content-state), so the widget needs no shared container or network.

**iOS (new build, 2.1):**
- New **Widget Extension target** `GoalDiggerLiveActivity` (bundle `…app.LiveActivity`). Added programmatically via `ios/scripts/add_live_activity_target.rb` (the `xcodeproj` gem) — **hand-editing `project.pbxproj` is too fragile**; the script is idempotent + re-runnable and wires the target, build settings, file membership, app dependency, and the Embed Foundation Extensions phase.
- `MatchActivityAttributes` (shared, both targets) + `LiveActivityWidget` UI (lock screen + Dynamic Island, brand colors). `NSSupportsLiveActivities` (+ FrequentUpdates) in the app Info.plist.
- `LiveActivityManager`: observes `pushToStartTokenUpdates` (17.2+) + each activity's `pushTokenUpdates`, registers both with the backend; foreground-start fallback via `live-match-current`. Wired into `AppDelegate` (launch) + `GoalDiggerApp` (scenePhase active).

**Backend:**
- Migration `062`: `live_activity_tokens` (anon-upsert posture mirroring device_tokens; opaque tokens, unusable without the .p8) + `la_started/la_sig/la_ended` drive-state columns on `match_status_state`.
- `sendLiveActivityPush` in `_shared/apns-client.ts` (reuses the ES256 JWT + host; `apns-push-type: liveactivity`, topic `…app.push-type.liveactivity`, the start/update/end payload).
- **match-watcher hooks** (WC only): START via push-to-start at kickoff, UPDATE on score/period change (period-based label, not minute-ticking — keeps APNs volume sane), END at FT. Idempotent via the `la_*` columns.
- `live-match-current` Edge Function for the foreground fallback.

**Verified live (kickoff day):** redeployed match-watcher ran clean against the live opener; `live-match-current` returned Mexico 2-0 South Africa (2H); the drive state engaged (`la_started=t`, `la_sig="2-0-2nd half"`). Pushes were no-ops only because no device has registered a token yet (the build isn't shipped). iOS app + extension build green.

**Ships in 2.1 + needs a physical device to fully verify** — push-to-start + remote ActivityKit updates can't be exercised on the simulator. Device-test path: install → app vends + registers the push-to-start token → with the app CLOSED, the activity appears at a followed country's kickoff, updates on goals/HT, ends at FT; open-app foreground fallback covers the rest. **Lessons: (1) a Live Activity, not a one-shot push, is the correct surface for live-match following — pushes are for discrete events, activities for evolving state; (2) add Xcode targets programmatically (xcodeproj gem), never by hand; (3) flag emoji sidesteps the widget's remote-image limitation entirely.**

### 100. WC live pushes (goal / half-time / full-time) for the two PLAYING teams

**Trigger:** WC opener (Jun 11). User had Mexico on one phone and South Africa on the other and got **zero pushes the whole game**. Root cause (from the DB + `pipeline_health`): at FT, match-watcher fires `gd-matchday` for both playing teams (HTTP 200) but **that routine produces nothing for country/WC entities** — it's PL-shaped. The only FT pushes that landed were `WC_RIVAL_RESULT` consequences, which by design go to the *group rivals* (Czech Republic, South Korea), not the teams that played. `detect-consequences.ts` even skips the playing teams ("covered by gd-matchday") — so they fell through the gap entirely. The user's call: **the two teams that play must get a push at every goal, at the break, and at full-time.** Group rivals stay after-the-game-only. Polling stays every-minute (the adaptive poll-gate was rejected: "pull it every minute so we don't mess it up").

**Design — three DIRECT APNs sends from match-watcher, not content_items.** Consequence rows wait on notification-sender's hourly sweep (5–75 min) — fine for rival results, useless for a live goal. These send inline from the tick that observed the event via `sendPushNotification` + `buildAPNsPayload`, to `device_tokens.country_id IN (home, away)`, all tiers (matches existing WC push behaviour — `news` type has `minTierForType=1`, no real gate). Deterministic, zero Claude. Templates in `_shared/goal-push.ts` (`renderGoalPush` already existed; added `renderHalfTimePush` + `renderFullTimePush`, perspective-aware — scorer celebrates / conceder commiserates, score shown follower-first). Works for **any round incl. knockouts** — it keys off the live score, not group math, so the unscheduled slutspel needs no per-game config. Distinct from the Lesson 99 Live Activity (persistent lock-screen score) — these are the one-shot alert banners.

**Idempotency, three mechanisms:**
- **Goal** — none needed. `detectGoal(prior.home/away_goals, current)` only returns a side when the score rose; the end-of-tick upsert advances the stored score, so the next tick sees no change. `prior !== null` guards the first-observation phantom.
- **HT** — `"HT_PUSH"` marker, kept in a SEPARATE `pushMarkers` array (NOT `newTriggers`). `newTriggers` drives the paid `gd-live-brief` routine fire loop, so a marker in it would fire a spurious billed routine (CLAUDE.md hard rule). Both arrays merge into `briefs_fired` for the once-per-window guard.
- **FT** — `"FT_PUSH"` marker, independent of the `gd-matchday` cap / `fired_finished_at` logic (that path no-ops for WC).

**The gotcha that bit on deploy (caught + fixed in the same session).** First version gated HT/FT only on the absent marker. On the very first tick of new code, the already-finished Mexico-SA game matched (`status=FT`, no `FT_PUSH` yet) and **fired a real, accurate-but-late FT push to the user's two phones ~1.5h after the whistle.** This is the classic deploy-mass-fire: any in-flight/just-finished state re-fires when new push logic ships. The existing matchday path already prevents this with a first-observation guard (`prior !== null && !prior.fired_finished_at`); the new pushes needed the same idea: **fire only on an OBSERVED transition.** FT now requires `!FINISHED_STATUSES.has(prior.status)` (live→finished); HT requires `prior.status !== "HT"` (→ break). Goal never had the problem (it diffs scores, which already match on deploy). Confirmed no re-fire post-fix.

**Verified:** 11/11 `goal-push.test.ts`; `deno check` clean (also fixed a pre-existing `Team` type mismatch in the consequence loop — `short_name` nullable + `entity_type` free string vs the `Team` interface; renderConsequence only reads `display_name`, so coerced harmlessly); all 48 WC countries in `WC_COUNTRY_META`; live smoke ticks return the new `goal_push_sends` field and run clean. Real end-to-end proof is the next live WC game (push at each goal, the break, the whistle) — also exercises the night-game UTC fix (22:00–23:30 UTC kickoffs, e.g. Brazil Jun 13).

**Lessons: (1) a transition-triggered push MUST gate on an observed state change (prior-status differs), never just on "haven't fired yet" — otherwise it mass-fires every already-in-that-state row on the deploy tick; smoke-test against a real recently-finished fixture, not just for a clean 200. (2) Keep direct-push idempotency markers OUT of any array that drives a billed routine fire. (3) When a downstream routine silently no-ops for a whole entity class (gd-matchday for countries), the deterministic layer must cover those entities itself — don't assume the routine "has it."**

### 101. WC match-result feed content: rival items polluted "Football", playing teams got nothing

**Trigger:** user opened the app after the opener and saw, under the "Football" feed (= the `everyone_talking` / "Everyone's talking about" context, `FeedContext.displayName`), TWO near-duplicate items — "Mexico beat South Africa 2-0, in Czech Republic's group" and the same "...in South Korea's group" — with **no talking points**, while the teams that actually played (Mexico, South Africa) had **no feed news at all** (tapping the FT push opened nothing).

**Root cause — the result-content model was inverted for WC.** match-watcher's FT path fires `gd-matchday` for the playing teams, but that routine **produces nothing for country entities** (Lesson 100); the only deterministic FT content was `detect-consequences`' `WC_RIVAL_RESULT`, which by design targets the *group rivals* and was inserted with **`everyone_talking=true`** — so the two rival rows leaked into the shared Football feed (one per rival = the duplicate), framed "in <rival>'s group" which is meaningless out of that rival's context, and with no `talking_points` (the consequence template only emits headline/body/push). Meanwhile the playing teams, whom `detect-consequences` deliberately SKIPS ("covered by gd-matchday"), got nothing.

**Decision (user):** richer LLM content was the first choice, but the `gd-matchday` routine is correctly configured (right prompt, enabled, valid token) yet **silently produced nothing on opening night and its claude.ai run logs aren't visible from the CLI** — undebuggable + unguaranteeable from here. Given the reliability priority, switched to a **deterministic floor** (LLM can enrich later once the routine's silent failure is diagnosed).

**Fix (match-watcher, deterministic, $0):**
- At FT of a WC game, write a real feed article per PLAYING team, reusing `renderPostMatch` (the same group-/best-third-aware prose the team-page card already uses) → `headline` (perspective: "Mexico beat South Africa 2-0" / "South Africa lost 0-2 to Mexico"), `body`, and **`talking_points: [pm.talking_point]`**. `push_eligible=false` (the FT push is sent directly; this must not re-push via notification-sender's sweep).
- The **HOME team's row alone** carries `everyone_talking=true` with a NEUTRAL `everyone_talking_headline` → the Football feed shows exactly ONE result, no perspective duplicate.
- `WC_RIVAL_RESULT` flipped to **`everyone_talking=false`** → stays on the rival's own feed (useful group context for *that* fan), out of Football.
- FT push now **deep-links** to the team's article (the post_match block runs a few steps before the push in the same tick, so the new content_item id is captured into `wcResultItemIds` and passed as the push `content_id`).
- **Stopped firing `gd-matchday` for WC entirely** — it no-ops for countries, so it was pure wasted routine quota (which busy WC days need for `gd-live-brief`); the loop now marks the perspective OK and skips the fire, so the deterministic block + `fired_finished_at` still run.
- One-time SQL backfill for the opener: rival rows → `everyone_talking=false`, plus the two missing Mexico/South Africa articles, so the live app was corrected without waiting for the next game.
- Minor: the default post-match `talking_point` was "the last round of games" (misleading after matchday 1) → "their next game".

**Lessons: (1) `everyone_talking` is a FEED-SCOPE flag, not an importance flag — a per-rival row marked everyone-worthy becomes N near-duplicates in the shared feed; the cross-team feed needs ONE neutral row, perspective rows stay team-scoped. (2) An entity the consequence layer SKIPS because "the routine covers it" gets nothing the moment that routine no-ops for that entity class — the floor must be deterministic. (3) Deep-linking a push to async-generated content works only if the content is written earlier in the SAME synchronous tick; capture the id then, not from a later routine.**

### 102. WC live-push copy variation: 40 takes per bucket, randomly picked at fire-time

**Trigger:** after the South Korea game, the pushes worked but each bucket had exactly ONE body string, so every goal read "{team} score! He's celebrating, good moment to text him" and every win read the same line. Over a tournament of dozens of games it reads like a robot. User wanted ~40 distinct human takes per bucket, shown at random, "or else it will sound generic when we send out."

**Design — pool + pick, server-side only.** New `_shared/goal-push-copy.ts` holds NINE 40-deep string pools (`GOAL_SCORED` / `GOAL_CONCEDED` / `GOAL_BOTH`, `HT_AHEAD` / `HT_BEHIND` / `HT_LEVEL`, `FT_WIN` / `FT_LOSS` / `FT_DRAW`) = 360 lines, with `{flag}`/`{team}`/`{score}`/`{home}`/`{away}` placeholders. `goal-push.ts` gained `pick(pool, rng=Math.random)` + `interpolate(template, vars)`; the three render fns now draw a variant at fire-time (independently per side) and the args object takes an optional `rng?` (default `Math.random`, tests inject `() => 0`). **Perspective stays correct by construction, not by copy** — `renderGoalPush` routes the SCORED pool to the scorer's slug and CONCEDED to the conceder's; HT/FT route lead/trail/level by the follower's own goal diff, score rendered follower-first. Titles unchanged (factual scoreline carries the score). `Math.random` is fine here — this is the Deno edge runtime, not a Workflow script. No migration, no iOS build; reaches everyone on the match-watcher redeploy.

**Verified:** 14/14 `goal-push.test.ts`, incl. a **sweep over all 360 lines** (each renders ≤90 chars worst-case with the longest name + England's tag-flag + a two-digit score, contains the score, no em/en dashes) and a **pool-integrity** check (each pool is exactly 40 with no exact-duplicate line). `deno check` clean; match-watcher deployed; eyeballed live output (England scores → fan gets a celebratory line, the opponent's fan a muted one, every draw a different take).

**Voice by bucket (user-directed refinement).** The brand is "his mood is HER strategic window," so the tone is tuned per moment, not uniform: **goals** stay short in-the-moment reactions (he's still glued to a live match, no point nudging a conversation); **HT ahead** = teasing the smug/superstitious winner; **HT behind** = grumpy-man + armchair-manager comedy ("he'd have made five changes, none of them legal"); **HT level** = coin-flip-mood / armchair / superstition / fake-calm; **FT win** = the cash-in ("he'd say yes to anything for the next hour, ask about those tickets"); **FT loss** = the inverse, hold-the-ask ("file it for another day, bring snacks instead"). Goal-conceded / both / FT-draw stay gentle/neutral. Key insight: the opportunistic "ask him now" voice belongs at FULL-TIME (game over, sustained good mood), not at a goal (he's still watching).

**Deliberate non-goals:** no anti-repeat guard (1/40 collision, and it would need new DB state); `GOAL_BOTH` rarely fires (two goals in one 1-min poll) but got 40 anyway per the ask.

**Lessons: (1) randomized copy keeps perspective safe ONLY if the wiring routes the right pool to the right recipient — never lean on the text to self-identify whose goal it was. (2) A sweep test that renders every variant worst-case (longest name + widest flag + two-digit score) is the cheap guarantee that no one of 360 hand-written lines busts the char cap or sneaks in a dash. (3) `Math.random`/pool-pick belongs in the edge function, not a Workflow script (where it is unavailable) — know which runtime the "deterministic" helper actually runs in.**
