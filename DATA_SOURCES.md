...
| `/players?team=&season=&page=` | appearances, minutes, starts, goals, assists, cards, saves, nationality, age per player | Fetched once a day (~40 calls), synced by `sync_player_stats_from_raw()` (migration 095) into `players.appearances` / `players.minutes`. Sane on 2026-09-08: Arsenal's four ever-presents at 364 minutes, the bench in double digits. **`statistics` is an array with one entry PER COMPETITION**, not a season total — `statistics[0]` is whatever competition the feed lists first (the Community Shield, for Arsenal), so sum the entries whose `team.id` is the club's. Only players with a stats record come back (22 of Arsenal's 39 squad rows), and `paging.total` is honest. Since migration 100 (2026-09-09) the counting stats are synced too, summed per club; `league_goals`/`league_starts` are league 39 only. **`games.rating` is the feed's own model** (a minutes-weighted mean lands in `players.rating`): show it as "rated 7.3 by the stats feed", never as a fact about form. `games.captain` was false for every Arsenal player on 2026-09-09 including Ødegaard, so a false there proves nothing. |
| `/injuries` | who is out | Club-correct: every record for a club names that club's own players. See the caveat below on shape. |
| `/predictions?fixture=` | both sides' form and season record (clean sheets, failed to score, biggest win and loss, penalties, formations lined up in), and the previous meetings between the two clubs | Re-enabled 2026-09-23 for the matchup card. The factual half agrees with `/standings` and `/fixtures`: checked for Arsenal v Leeds, Arsenal's form and 3 clean sheets matched the table. **Its `predictions` block is the feed's own model, not a fact** — see below. |
...
**`/predictions` is one call per fixture and carries betting copy.** It is keyed by fixture, not by team, so `data-fetcher` takes the id out of the `fixtures_next` payload of the same run and **skips the call when `raw_fetch_logs` already holds a row for that fixture** — roughly one call per club per matchweek instead of one every two hours. The feed's response does not name the fixture it was bought for, so the stored row is wrapped: `{ "fixture_id": <id>, ...payload }`. The twenty rows from 2026-04-28 predate the wrapper and have no `fixture_id`, which is why the gate treats them as absent.

Three things inside it are not facts and one of them is not even neutral. `predictions.percent` and `predictions.goals` are the feed's own model, in the same class as `games.rating`. `predictions.advice` is literally a betting tip ("Double chance : draw or Sunderland"). **None of the three ever reaches a card.** `buildMatchupCard` (`_shared/matchup-card.ts`) copies only `predictions.winner.id`, collapsed to `us` / `them` / `even`, and the app uses it to pick a tone rather than to show a number. There is a test asserting the percentages and the advice string cannot appear in the output.

The `h2h` array is **the whole history and is not in date order** — a 2025 meeting came back ahead of a 2018 one. Sort it yourself and filter to meetings that have a score. `league.form` is the season-long string (34 characters in April), not the last five.
...
**Player photos rot the same way, and the "all 781 were real" line above expired.** Re-checked on 2026-09-23 over all 621 photos for the 20 active clubs: **55 of them (8.9%)** are one shared placeholder, md5 `430d67fd79ad0a355b212d5780886e34`, 5192 bytes, a grey circular silhouette. Sunderland has 11 of them, Leeds and Crystal Palace 5 each. They cluster on players signed since the last check, which is the same pattern as the managers. This one is left in place: unlike the manager placeholders it reads as a deliberate avatar rather than a broken image, and `players.photo_url` is rewritten nightly from the squad payload by `sync_players_from_squads()`, so nulling it would not survive the night. Count it, do not claim every player has a face.

**API-Football HTML-escapes apostrophes in names.** `/players/squads?team=50` returns `N. O&apos;Reilly`, not `N. O'Reilly` — verified against the live endpoint on 2026-09-23, so it is the feed and not our ingest. Present in `api_football_squad`, `api_football_transfers`, `api_football_players_stats` and the `mirror` source; not seen in the events payload, which is the one that names a scorer in a push. Six current players carry it. It is decoded on the way into `players` by migration 110 (`decode_feed_entities()`, `&amp;` decoded last so a doubly-escaped entity does not corrupt), so read names from that table rather than from a payload. Anything that compares a name against the RAW snapshot has to decode first: `post_team_page.sh`'s grounding guard did not, so a card correctly naming O'Reilly was rejected for naming a player who is in the squad, and the club's whole payload with it.
...
- **`club_style` (migration 112) has no feed behind it at all.** How a side plays — set pieces, the counter, aerial duels, where they score from, which side they attack — is not in API-Football in any form, so the table is hand-seeded from published league tables and nothing writes it but a person. `NULL` means unverified and renders as false; `false` means checked and untrue. A manager change invalidates every claim about that club, which is why the audit checks it against `teams.manager_started_on`.
