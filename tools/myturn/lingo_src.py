...
RULES = [
    ...
    ("handball", "Handball", "Touching the ball with your hand or arm on purpose, or with an arm stuck out making you bigger. Goalkeepers can use their hands, but only inside their own box.", "\"That's handball all day\" about a ball that brushed a shoulder.", "\"Was his arm out? That's handball if it was.\"", ["the-box"]),
    ...
    ("yellow-card", "Yellow card", "A warning from the referee. Two in one game and you are sent off.", "\"He's on a yellow\" means the player has to be careful now.", "\"Is he on a yellow? One more and he's off, isn't it?\"", ["red-card"]),
    ...
    ("clean-sheet", "Clean sheet", "Letting in no goals in a match. The goalkeeper's favourite statistic, and a defender's.", "\"Another clean sheet\" said with real pride.", "\"That's a clean sheet. Have they kept many this season?\"", []),
    ...
    ("league-cup", "League Cup", "The League Cup (Carabao Cup) is the smaller of the two domestic cups, so it takes the name of whichever sponsor is paying. Semi-finals over two legs, final at Wembley.", "\"Good chance to rest people\" from the manager.", "\"It's the League Cup. Will he rest half the team?\"", ["the-cups", "wembley", "two-legs"]),
...
TACTICS = [
    ...
    ("back-four", "Back four", "The four defenders in a line in front of the goalkeeper: two centre-backs in the middle, a full-back on each side.", "\"The back four were solid\" is high praise.", "\"Have the back four been solid today? Looked alright to me.\"", ["full-back", "centre-back"]),
    ("centre-back", "Centre-back", "The two big defenders in the middle. Heading, tackling, shouting at everyone else.", "\"He's a proper centre-back\" means old-fashioned and tough.", "\"Is he a proper centre-back? Does he win his headers?\"", []),
    ...
    ("wing-back", "Wing-back", "A full-back who plays higher up the pitch, almost as a winger, because there are three centre-backs behind him.", "When a team plays with a back three.", "\"Are those wing-backs? That means a back three, doesn't it?\"", ["full-back", "back-four"]),
    ...
    ("box-to-box", "Box-to-box", "A midfielder who runs from his own penalty area to the other one all game. Defends, attacks, never stops.", "\"Proper box-to-box midfielder\" said admiringly.", "\"Is he a box-to-box player, then? Does he get back as well?\"", ["holding-midfielder"]),
    ("number-ten", "Number ten", "The creative player just behind the striker who makes the clever passes. Named after the shirt number the best of them wore.", "\"He's a classic ten\" about someone who can't be bothered to defend.", "\"Is he playing as the ten today, just behind the striker?\"", ["playmaker"]),
    ("playmaker", "Playmaker", "The player everything goes through. Sees passes nobody else sees. Stop him and you stop the team.", "\"He makes them tick\" is the phrase.", "\"Which one's their playmaker? Who makes them tick?\"", ["number-ten"]),
    ...
    ("striker", "Striker", "The player whose whole job is scoring goals. Also called a forward, a centre-forward or a number nine.", "\"They need a striker\" about half the clubs in the league.", "\"Have they got a proper striker? Who plays up top for them?\"", ["false-nine", "target-man"]),
    ...
    ("false-nine", "False nine", "A striker who drops back into midfield instead of staying up front, so the defenders don't know who to mark.", "In clever tactical chats. Sounds fancier than it is.", "\"Is he playing as a false nine, dropping into midfield?\"", ["striker"]),
    ("pressing", "Pressing", "Chasing the opponent the moment he gets the ball, all over the pitch, to force a mistake. Exhausting when it works and when it doesn't.", "\"They press really well\" about energetic teams.", "\"Are they a pressing side, or is that just them chasing it?\"", ["high-press", "low-block"]),
    ...
    ("low-block", "Low block", "The whole team sitting deep near their own goal, letting the other side have the ball and hoping to hit them on the counter.", "\"They've sat in a low block\" about a team defending a lead.", "\"Are they sitting in a low block? Someone needs to unlock this.\"", ["park-the-bus", "counter-attack"]),
    ("park-the-bus", "Park the bus", "Defending with everyone and attacking with nobody, as if a bus were parked across the goal. Usually to protect a lead.", "Usually as an insult about a boring opponent.", "\"If they park the bus, this'll be hard work.\"", ["low-block"]),
    ("counter-attack", "Counter-attack", "Winning the ball and racing up the pitch fast before the other team can get back into position.", "\"Dangerous on the counter\" about quick teams.", "\"Are they dangerous on the counter? I'd not push too many forward.\"", ["on-the-break"]),
    ...
    ("long-ball", "Long ball", "Kicking the ball high and far up the pitch instead of passing it along the ground. Simple, direct, unloved.", "\"Route one\" is the same idea, said more rudely.", "\"Long ball again. Does that ever come off?\"", ["target-man", "route-one", "hoof"]),
    ...
    ("zonal-marking", "Zonal marking", "At corners and free kicks, each defender guards a patch of ground instead of a person. Blamed for every goal from a corner.", "Blamed for every goal from a corner.", "\"Is that zonal marking? Nobody's actually on anyone.\"", ["man-marking"]),
    ("set-piece", "Set piece", "Any restart from a dead ball: corners, free kicks, throw-ins. Teams rehearse them in training all week.", "\"Strong from set pieces\" about tall teams.", "\"Are they any good from set pieces? I'll watch the corners.\"", ["corner", "free-kick"]),
    ...
    ("inverted-full-back", "Inverted full-back", "A full-back who steps into midfield when his team has the ball, to give them an extra passer in the middle.", "Guardiola fans. Sounds very clever.", "\"Is the full-back inverting there? He looks like a midfielder.\"", ["full-back"]),
    ("half-space", "Half-space", "The gap between the middle of the pitch and the wing, where clever attackers like to get the ball because nobody is quite sure who marks them.", "Tactics podcasts. Nod if you hear it.", "\"Is that the half-space he keeps getting into? Nobody picks him up.\"", []),
...
MATCH_SITUATIONS = [
    ...
    ("second-ball", "Second ball", "After a header or a clearance, the loose ball that drops. Winning the second ball means reacting quickest to it.", "\"They won the second ball\" as praise for hard work.", "\"They keep winning the second ball. Is that why they're on top?\"", []),
...
CULTURE = [
    ("fergie-time", "Fergie time", "Generous added time that seems to appear whenever a big club needs a goal. Named for Alex Ferguson's Manchester United, who scored a lot of late winners.", "Sarcastically, by fans of every other club.", "\"Fergie time, is it? How long has he added?\"", ["added-time"]),
    ...
    ("hard-man", "Hard man", "A player famous for tough, physical, slightly frightening football. Tackles first, apologises never.", "\"Proper hard man\" about a centre-back or a midfielder who scares people.", "\"Is he a proper hard man? I'd not want to cross him.\"", ["centre-back"]),
    ("class-act", "Class act", "A player or manager admired for how they behave as much as how they play. Good on the pitch, better off it.", "About a player who does charity work or handles a loss with grace.", "\"Is he a class act, then? On and off the pitch?\"", []),
    ("top-top-player", "Top, top player", "Pundit-speak for very good. Two tops is better than one, and nobody knows why.", "Match of the Day, any week.", "\"Top, top player, is he? What makes him that good?\"", ["top-drawer"]),
    ...
    ("wembley", "Wembley", "England's national stadium, in north-west London. Both domestic cup finals are played there, and the FA Cup semi-finals too. Ninety thousand seats under an arch.", "\"A trip to Wembley\" means the cup run has gone deep.", "\"So a win here is a trip to Wembley?\"", ["the-cups", "fa-cup", "league-cup"]),
