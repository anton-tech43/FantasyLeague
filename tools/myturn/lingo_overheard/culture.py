# Overheard content for category "culture". See LINGO_OVERHEARD_BRIEF.md.
# Keep ids in lingo_src.py order. Every id in the category must appear here.

OVERHEARD = {
    "fergie-time": dict(
        overheard="Their lot always get the rub of the green. Fergie time, my dad calls it.",
        speaker="chat",
        gist="Stoppage minutes that suit the bigger side",
        decoy="Kick-off times chosen to suit the television",
        spare="The seasons a famous manager was in charge",
        when=["any", "after-loss", "title", "h2h-they-win"],
        moment="common",
    ),
    "the-gaffer": dict(
        overheard="Gaffer's seen it. Look at his face.",
        speaker="him",
        gist="The manager, in old workplace slang",
        decoy="The club captain who leads on the pitch",
        spare="The head of the backroom staff at a club",
        when=["any", "new-manager", "bad-run", "after-heavy-loss", "opp-bad-form"],
        moment="anytime",
        player=dict(
            slot='ours.keeper',
            overheard="You'd have to ask the gaffer about {ours.keeper}. I've got no idea.",
            sayIt='"What\'s the gaffer like with {ours.keeper}? Do you ever hear?"',
        ),
    ),
    "the-boss": dict(
        overheard="It'll be the boss's call and nobody will argue with it.",
        speaker="pundit",
        gist="The manager, the word the squad uses for him",
        decoy="The club owner who pays everyone's wages",
        spare="The senior player who speaks in the dressing room",
        when=["any", "new-manager"],
        moment="common",
        player=dict(
            slot='ours.midfielder',
            overheard="Whether {ours.midfielder} plays is the boss's call. Nobody else gets a say.",
            sayIt='"Is it the boss who decides whether {ours.midfielder} plays, or the coaches?"',
        ),
    ),
    "hard-man": dict(
        overheard="Signed a proper hard man in the summer. First start today.",
        speaker="chat",
        gist="A player feared for his fierce, physical game",
        decoy="A player who is difficult to deal with off it",
        spare="A defender who blocks shots with his body",
        when=["any", "derby"],
        moment="anytime",
    ),
    "class-act": dict(
        overheard="What a player he was. Class act, every time I saw him.",
        speaker="pundit",
        gist="A man admired for his conduct as much as his play",
        decoy="A player putting on a show for the cameras",
        spare="A polished touch that makes the game look easy",
        when=["good-run", "after-win"],
        moment="anytime",
        player=dict(
            slot='theirs.keeper',
            overheard="Class act, is he, {theirs.keeper}? I'd have to see more of him.",
            sayIt='"Is {theirs.keeper} a class act, or is that just what they say?"',
        ),
    ),
    "top-top-player": dict(
        overheard="He's a top top player and I don't say that lightly.",
        speaker="pundit",
        gist="Pundit-speak for about as good as it gets",
        decoy="Someone at the top of the scoring charts",
        spare="A man voted best on the pitch that day",
        when=["good-run", "after-win", "after-big-win", "window", "opp-good-form"],
        moment="anytime",
        player=dict(
            slot='theirs.midfielder',
            overheard='Would you call {theirs.midfielder} a top top player? Honestly, now.',
            sayIt='"Is {theirs.midfielder} a top top player, or is that just pundit talk?"',
        ),
    ),
    "match-of-the-day": dict(
        overheard="Match of the Day gets worse every year. I still watch it, mind.",
        speaker="him",
        gist="The BBC's Saturday night highlights show",
        decoy="The televised game chosen for Saturday teatime",
        spare="A round-up of the goals from abroad each week",
        when=["any", "after-win", "after-loss"],
        moment="anytime",
    ),
    "pundit": dict(
        overheard="The pundits said we'd be fine. That's the worry.",
        speaker="him",
        gist="An ex-player paid to give opinions on TV",
        decoy="A journalist who writes the match report",
        spare="An expert who predicts the score beforehand",
        when=["any", "after-loss"],
        moment="anytime",
        player=dict(
            slot='theirs.defender',
            overheard="Ask a pundit about {theirs.defender} and you'll get ten minutes of it.",
            sayIt='"What do the pundits actually say about {theirs.defender}?"',
        ),
    ),
    "the-lads": dict(
        overheard="Buzzing for the lads tonight. Been a long few weeks.",
        speaker="chat",
        gist="The team, as the players and fans call it",
        decoy="The younger players coming through the academy",
        spare="The travelling fans who follow every game",
        when=["any", "after-win", "good-run"],
        moment="common",
    ),
    "bottle-it": dict(
        overheard="I'm saying nothing yet. We'll bottle it, we always do.",
        speaker="him",
        gist="To lose your nerve at the crucial moment",
        decoy="To keep it all in and say nothing about it",
        spare="To waste chances in front of goal all game",
        when=["title", "run-in", "bad-run", "after-loss", "h2h-they-win"],
        moment="anytime",
        player=dict(
            slot='theirs.keeper',
            overheard="One of us is going to bottle it here. Might be us, might be {theirs.keeper}.",
            sayIt='"Who bottles it first, do you reckon? Us or {theirs.keeper}?"',
        ),
    ),
    "banter": dict(
        overheard="He's not answered since Saturday. Can't take the banter.",
        speaker="chat",
        gist="Teasing between fans, mostly good-natured",
        decoy="Chatter between the players on the pitch",
        spare="A row between fans that has gone too far",
        when=["any", "derby", "after-win", "h2h-we-win"],
        moment="anytime",
        player=dict(
            slot='ours.forward',
            overheard='The banter if {ours.forward} misses one of those, though. Imagine.',
            sayIt='"Is that banter, or would you actually fall out over {ours.forward}?"',
        ),
    ),
    "group-chat": dict(
        overheard="Don't mind me, I'm just catching up on the group chat.",
        speaker="him",
        gist="Where his mates react to every goal instantly",
        decoy="An online forum open to all supporters",
        spare="Fans posting videos from inside the ground",
        when=["any", "derby", "after-win", "after-loss", "h2h-we-win"],
        moment="anytime",
    ),
    "season-ticket": dict(
        overheard="My dad had that season ticket before me. No chance I'm giving it up.",
        speaker="him",
        gist="A pass covering every home league game",
        decoy="A pass that gets you into any ground you like",
        spare="A membership that lets you buy cup tickets early",
        when=["any", "early-season"],
        moment="anytime",
    ),
    "away-day": dict(
        overheard="Big away day Saturday. I'll be useless by Sunday.",
        speaker="chat",
        gist="A trip to watch the team at another ground",
        decoy="A day off the players get away from the club",
        spare="A pre-season friendly played overseas",
        when=["any", "cup", "europe"],
        moment="anytime",
    ),
    "the-away-end": dict(
        overheard="Second half and they're attacking the away end now.",
        speaker="telly",
        gist="The part of the ground given to visiting fans",
        decoy="The end of the pitch the visitors attack",
        spare="The stand behind the goal at the far side",
        when=["any", "cup", "europe", "derby"],
        moment="common",
    ),
    "kop": dict(
        overheard="The Kop are on their feet and they've not sat down for ten minutes.",
        speaker="telly",
        gist="Liverpool's loud home stand at Anfield",
        decoy="The steepest old stand, where tickets are cheap",
        spare="A stand named after a club's great manager",
        when=["any"],
        moment="rare",
    ),
    "terraces": dict(
        overheard="My old man swore by the terraces. He'd not enjoy it now.",
        speaker="pundit",
        gist="The old standing areas, and the fans in them",
        decoy="The rows of houses behind an old ground",
        spare="The banked rows of seating in the top tier",
        when=["any", "derby", "h2h-we-win"],
        moment="common",
    ),
    "chant": dict(
        overheard="The whole ground was on that chant tonight. Goosebumps.",
        speaker="him",
        gist="A song the crowd sings to a pop tune",
        decoy="A rhythm the crowd claps without words",
        spare="The song a club plays as the teams come out",
        when=["any", "derby", "cup", "h2h-we-win"],
        moment="common",
    ),
    "half-and-half-scarf": dict(
        overheard="Bloke in front of us in a half-and-half scarf. Unbelievable scenes.",
        speaker="chat",
        gist="A souvenir showing both teams' colours",
        decoy="A ticket that covers both legs of a round",
        spare="A scarf swapped with a rival fan at full time",
        when=["derby", "cup", "h2h-we-win"],
        moment="anytime",
    ),
    "plastic-fan": dict(
        overheard="Listen to them. Plastic fans, the lot of them.",
        speaker="him",
        gist="A fan who only turns up when things go well",
        decoy="Someone who turns up in a shop-bought kit",
        spare="A supporter who watches on TV and nothing else",
        when=["any", "derby", "good-run", "opp-good-form"],
        moment="anytime",
    ),
    "glory-hunter": dict(
        overheard="Glory hunter, him. Always has been, even at school.",
        speaker="chat",
        gist="A fan who picked a club for its trophies",
        decoy="A player chasing his own goals over the team",
        spare="Someone who collects club shirts and badges",
        when=["any", "good-run", "title", "opp-good-form"],
        moment="anytime",
    ),
    "the-boot-room": dict(
        overheard="The boot room was a proper thing once. All gone now.",
        speaker="pundit",
        gist="Promoting a manager from inside the club",
        decoy="The place at a ground where kit is stored",
        spare="A club's academy for teenage players",
        when=["any", "new-manager"],
        moment="rare",
    ),
    "the-invincibles": dict(
        overheard="Their lot will bring up the Invincibles before the first goal.",
        speaker="chat",
        gist="A whole league season without a defeat",
        decoy="A team that went a season without losing at home",
        spare="A side that won three trophies in one year",
        when=["title", "good-run", "opp-good-form"],
        moment="anytime",
    ),
    "sack-race": dict(
        overheard="He'll be in the sack race by Christmas at this rate.",
        speaker="chat",
        gist="The unofficial contest over which manager goes first",
        decoy="A charity event before a game at the ground",
        spare="The run of games that decides a manager's future",
        when=["bad-run", "new-manager", "after-heavy-loss", "opp-bad-form"],
        moment="anytime",
    ),
    "new-manager-bounce": dict(
        overheard="You'll hear a lot about the new manager bounce this week.",
        speaker="pundit",
        gist="The short run of good results after a change",
        decoy="The lift in ticket sales after an appointment",
        spare="The boost a home crowd gives a struggling side",
        when=["new-manager", "good-run", "opp-good-form"],
        moment="rare",
    ),
    "silly-season": dict(
        overheard="Silly season, that's all this is. Same every year.",
        speaker="him",
        gist="Summer, when every rumour gets printed",
        decoy="The run of games over Christmas and New Year",
        spare="A spell when results make no sense at all",
        when=["window", "early-season"],
        moment="anytime",
    ),
    "here-we-go": dict(
        overheard="Here we go, apparently. My phone hasn't stopped.",
        speaker="chat",
        gist="The reporter's signal a transfer is done",
        decoy="The chant fans start as a big game kicks off",
        spare="The countdown to the window shutting",
        when=["window"],
        moment="anytime",
    ),
    "football-twitter": dict(
        overheard="Football Twitter has been at it since lunchtime. Not for me.",
        speaker="him",
        gist="The online corner where fans argue all day",
        decoy="A club's official account posting the news",
        spare="The reporters who break transfer stories",
        when=["any", "window", "after-loss"],
        moment="anytime",
    ),
    "fantasy-football": dict(
        overheard="Don't talk to me about fantasy football this week.",
        speaker="him",
        gist="A game where you pick real players for points",
        decoy="A made-up league played on a games console",
        spare="Betting on which players will score first",
        when=["any", "early-season"],
        moment="anytime",
        player=dict(
            slot='ours.forward',
            overheard="Fantasy football's ruining my life. Ask me about {ours.forward} and see.",
            sayIt='"Is {ours.forward} good, or is that your fantasy football team talking?"',
        ),
    ),
    "we-go-again": dict(
        overheard="We go again Tuesday. Kick-off's at eight, apparently.",
        speaker="chat",
        gist="Bad result, forget it, on to the next game",
        decoy="The match is being replayed after a draw",
        spare="Fans are heading off to another away trip",
        when=["after-loss", "after-heavy-loss", "bad-run", "after-draw", "opp-bad-form"],
        moment="common",
    ),
    "the-lino": dict(
        overheard="Lino! That's never a throw to them. He's making it up now.",
        speaker="him",
        gist="The assistant referee who judges offside",
        decoy="The painted markings around the pitch",
        spare="The fourth official holding up the board",
        when=["any"],
        moment="common",
    ),
    "wags": dict(
        overheard="My mum still goes on about the WAGs at that 2006 World Cup.",
        speaker="chat",
        gist="Tabloid slang for players' wives and girlfriends",
        decoy="Photographers who chase the players about",
        spare="The section of the stand kept for families",
        when=["any"],
        moment="anytime",
    ),
    "magic-sponge": dict(
        overheard="Here comes the magic sponge. We've all seen this before.",
        speaker="telly",
        gist="A wet flannel that fixes any knock",
        decoy="A cloth used to dry the ball",
        spare="The stretcher carried on for a bad injury",
        when=["any"],
        moment="common",
    ),
    "goal-of-the-month": dict(
        overheard="Goal of the month's going to be a tough one this time.",
        speaker="pundit",
        gist="The best strike, picked by a viewers' vote",
        decoy="The award for a player's form over four weeks",
        spare="A scoring target a striker is set each month",
        when=["any", "after-win", "after-big-win", "opp-long-range"],
        moment="rare",
    ),
    "the-magic-of-the-cup": dict(
        overheard="You cannot write this. The magic of the cup, that is.",
        speaker="telly",
        gist="The idea that a small club can beat a big one",
        decoy="A trophy handed over on the pitch at the end",
        spare="The extra money a cup run brings a club",
        when=["cup", "underdog"],
        moment="rare",
    ),
    "wembley": dict(
        overheard="Two games from Wembley. Don't say it out loud, don't jinx it.",
        speaker="him",
        gist="The national stadium in north-west London",
        decoy="The name fans give to the cup final itself",
        spare="The training base the national team uses",
        when=["cup", "run-in"],
        moment="rare",
    ),
    "the-third-round": dict(
        overheard="We've got them in the third round. Of course we have.",
        speaker="him",
        gist="The FA Cup stage in January the big clubs enter",
        decoy="The third game of a group in a tournament",
        spare="The round of matches played on Boxing Day",
        when=["cup"],
        moment="rare",
    ),
}
