...
LINE_CAP = 80        # the contract stores 120; the push has to fit it alongside the goal copy
LINE_MIN = 12
MIN_BANKERS = 8
...
def validate_calls(calls, *, err, warn, check_idiom, lingo_ids: set) -> int:
    ...
    for c in calls:
        ...
        n = len(line)
        if n > LINE_CAP:
            err(f"{where}: line is {n} chars (cap {LINE_CAP}, the push has to carry it): {line[:50]}…")
        if n < LINE_MIN:
...
if __name__ == "__main__":
    ...
    only(dict(ok, id="x", line="A goal " + "and another " * 8 + "one."), "cap 80")
